...
### Fase 3 — narrativa da montanha

| Ponto do blueprint                                 | O que foi feito                                                                                                 | Motivo                                                                                                                                                                                                                                        |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Usar o SVG oficial do símbolo"                    | **Não atendido — pendência registrada.** A timeline roda sobre a reconstrução medida de `lib/brand/mountain.ts` | O vetor oficial não foi entregue. A reconstrução foi medida na máscara de pixels e verificada por sobreposição, não é vetorização automática. Agora consta em `pendingRegistry`, então `npm run check:launch` falha enquanto o SVG não chegar |
| "palco fixado"                                     | `position: sticky`, não `pin` do ScrollTrigger                                                                  | O `pin` insere um pin-spacer e remede a página: é fonte de layout shift e de erro no refresh no meio da página. Sticky é do navegador, não desloca nada e já entrega o fallback estático de graça. O ScrollTrigger fica só com o scrub        |
| 220vh a 280vh                                      | 280vh no desktop, 200vh no tablet                                                                               | O tablet tem menos percurso porque o texto ocupa proporcionalmente mais altura                                                                                                                                                                |
| "linhas topográficas"                              | Quatro linhas na altura exata de cada beat, tiradas de `pointAtProgress`                                        | Derivar da geometria em vez de chutar valores de y                                                                                                                                                                                            |
| Altitudes dos beats                                | Ajustadas para 0.10 / 0.38 / 0.64 / 1.00                                                                        | O cume fica em 0.679 do traço. Com os valores antigos, `exposicao` caía em cima do cume e `patrimonio` já era descida. Agora a passagem de exposição para patrimônio é a travessia do ponto mais alto                                         |
| Mobile "quatro blocos verticais com o SVG ao lado" | Feito, e o texto nunca é escondido                                                                              | Só o traço é animado. Esconder texto para revelar depois deixaria o conteúdo refém de um gatilho                                                                                                                                              |

### Ajuste fora de fase — fundo fotográfico do hero

Pedido direto do cliente, fora da numeração de fases: substituir o fundo do
hero (antes: cor vinho plana + linha fina da montanha) por duas fotografias
fornecidas por ele — paisagem para desktop, retrato para mobile.

| Ponto do blueprint                                                                                 | O que foi feito                                                                                                                                                           | Motivo                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Nada de foto de escritório no hero. A identidade deve ser o primeiro impacto."                    | Mantido no espírito: as fotos são paisagem de montanha, não escritório, e a linha fina da montanha (que fazia esse papel) foi removida do hero para não duplicar o motivo | Decisão explícita do cliente, que se sobrepõe à direção original quando conflita                                                                                                                                                                                                                                                                                                                                                    |
| "sem simular uma montanha fotográfica literal" (blueprint, seção 4, sobre a _sequência_ da subida) | Não se aplica ao hero — essa regra é sobre a sequência narrativa da subida (`components/sections/Ascent.tsx`), que continua em linha abstrata, sem imagem                 | A regra nunca proibiu fotografia em outras seções, só que a sequência da subida não virasse uma montanha fotográfica                                                                                                                                                                                                                                                                                                                |
| Duas imagens diferentes por breakpoint (não a mesma redimensionada)                                | `getImageProps` do Next.js montando um `<picture>`/`<source>` nativo — o padrão que a própria documentação do Next recomenda para direção de arte responsiva              | `next/image` sozinho varia o tamanho de um arquivo; não troca de arquivo por media query                                                                                                                                                                                                                                                                                                                                            |
| Contraste AA sobre imagem com pontos de brilho reais (relevo iluminado da rocha)                   | Scrim em duas camadas: base uniforme (56% vinho) + reforço radial atrás da coluna de texto (até 88% vinho no centro)                                                      | Uma camada uniforme sozinha não segurava contraste nos pontos mais claros da foto sem apagar a imagem inteira. Calibrado renderizando a página com o texto tornado transparente e medindo, pixel a pixel, toda a área de cada elemento de texto — não por amostragem pontual, que se mostrou enganosa (pixels de borda antisserrilhada do próprio texto contaminavam a medição). Pior caso medido: 4,94:1, acima do mínimo de 4,5:1 |

Arquivos: `public/images/hero/hero-desktop.webp`, `public/images/hero/hero-mobile.webp`,
`components/sections/HeroBackground.tsx`, `styles/hero.css`.
